...
// 成员（为简化描述起见统一开放，读者可根据需要进一步封装）
    ...
    int height; // 高度（通用）
    int npl; // Null Path Length（左式堆，也可直接用 height 代替）
...
template <typename T>
BinNodePosi(T) BinNode<T>::insertAsLC(T const& e) { // 将 e 作为当前节点的左孩子插入二叉树
    return lChild = new BinNode(e, this);
}

template <typename T>
BinNodePosi(T) BinNode<T>::insertAsRC(T const& e) { // 将 e 作为当前节点的右孩子插入二叉树
